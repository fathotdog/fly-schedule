...
public record BatchTeacherAssignmentRequest(int TeacherId, List<BatchTeacherAssignmentItem> Upserts, List<int> DeleteIds, bool Force = false);
...
public record CopyCourseAssignmentsToGradeResponse(int TargetClassCount, int Created, int Skipped);

public record AssignTeacherRequest(List<int> AssignmentIds, int TeacherId, bool Force = false);
