...
import { getTimetable, getPeriods, createTimetableSlot, getSpecialRooms } from '@/api/client';
...
export function ClassSchedulePanel() {
  ...
  const { data } = useQuery({
    queryKey: ['timetable', currentSemesterId, 'class', selectedClassId],
  ...
  if (!selectedClassId || !data) {
    return (
      <div className="bg-surface-container-low rounded-2xl p-8 text-sm text-center flex flex-col items-center gap-2">
        ...
        <span className="text-on-surface-variant">點擊班級名稱查看課表</span>
      </div>
    );
  }
  ...
  const availableAssignments = data.courseAssignments.filter(ca => ca.scheduledPeriods < ca.weeklyPeriods);
    ...
    <div>
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="font-medium text-on-surface">
          {data.slots[0]?.classDisplayName ?? '班級'} 的課表
        </h3>
                  ...
                  >
                    {slot && (
                      <div>
                        ...
                        <div className="text-on-surface-variant">{slot.teacherName}</div>
        ...
        >
          ...
          <div className="space-y-1 max-h-56 overflow-auto">
            {availableAssignments.length === 0 ? (
              <div className="text-center text-on-surface-variant text-sm py-4">此班級所有配課皆已排滿</div>
            ) : (
              availableAssignments.map(ca => (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-on-surface-variant">{ca.teacherName}</span>
                  </div>
